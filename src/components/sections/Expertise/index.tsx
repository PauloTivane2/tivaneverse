...
export function Expertise() {
  ...
  return (
    <section id="expertise" className="corporate-section bg-light-background dark:bg-dark-background relative" ref={ref}>
      ...
      <div className="corporate-container relative z-10">
        ...
        >
                ...
                return (
                  <motion.button
                    ...
                    className={`
                      ...
                      ${isActive 
                        ? 'bg-gradient-to-r from-light-primary dark:from-dark-primary via-light-primary dark:via-dark-primary to-light-secondary dark:to-dark-secondary text-light-background dark:text-dark-background shadow-lg scale-[0.98] sm:scale-100' 
                        : 'bg-light-foreground/5 dark:bg-dark-foreground/5 text-light-foreground/70 dark:text-dark-foreground/70 hover:bg-light-foreground/10 dark:bg-dark-foreground/10 hover:text-light-primary dark:text-dark-primary border border-foreground/10'
                      }
                    `}
                  ...
                  >
                    ...
                    {count > 0 && (
                      <span className={`
                        ...
                        ${isActive ? 'bg-light-background/20 dark:bg-dark-background/20 text-light-background dark:text-dark-background' : 'bg-light-primary/10 dark:bg-dark-primary/10 text-light-primary dark:text-dark-primary'}
                      `}>
                        {count}
                      </span>
                    )}
          ...
          <div className="sm:hidden text-center mt-3 text-xs text-light-accent/60 dark:text-dark-accent/60">
          ...
          {loading ? (
            ...
            Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="flex-shrink-0 w-[85vw] sm:w-[70vw] md:w-[450px] snap-center">
                <div className="animate-pulse p-5 sm:p-6 rounded-xl bg-light-background dark:bg-dark-background border border-foreground/10 h-full">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="w-12 h-12 sm:w-14 sm:h-14 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded-lg flex-shrink-0"></div>
                    <div className="flex-1 space-y-2">
                      <div className="h-5 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded w-3/4"></div>
                      <div className="h-3 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded w-1/2"></div>
                    </div>
                  </div>
                  <div className="space-y-2 mb-4">
                    <div className="h-2 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded"></div>
                    <div className="h-3 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded w-16"></div>
                  </div>
            ...
            >
              <div className="corporate-card p-4 sm:p-5 md:p-6 h-full flex flex-col hover:border-light-primary/30 dark:border-dark-primary/30 transition-all duration-300 active:scale-[0.98] sm:active:scale-100">
                {/* Header com Icon e Título */}
                <div className="flex items-start gap-3 sm:gap-4 mb-3 sm:mb-4">
                  {/* Icon Container */}
                  <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-14 md:h-14 rounded-lg bg-light-primary/10 dark:bg-dark-primary/10 flex items-center justify-center flex-shrink-0 transition-transform duration-300 group-hover:scale-110">
                    {skill.iconUrl ? (
                      <div
                        className="w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 bg-light-primary dark:bg-dark-primary"
                        style={{
                      ...
                      <skill.icon className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-light-primary dark:text-dark-primary" />
                    )}
                  </div>

                  {/* Título e Categorias */}
                  <div className="flex-1 min-w-0">
                    <h3 className="text-sm sm:text-base md:text-lg font-bold text-light-foreground dark:text-dark-foreground mb-1 group-hover:text-light-primary dark:text-dark-primary transition-colors leading-tight">
                      {skill.name}
                    </h3>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {skill.categories.map((category) => {
                          ...
                          return (
                            <span 
                              key={category}
                              className="inline-flex items-center gap-1 px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs font-medium bg-light-foreground/5 dark:bg-dark-foreground/5 text-light-accent dark:text-dark-accent rounded"
                            >
                  ...
                  {skill.featured && (
                    <div className="flex-shrink-0 hidden sm:flex">
                      <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-light-primary/10 dark:bg-dark-primary/10 flex items-center justify-center">
                        <FiStar className="w-3.5 h-3.5 text-light-primary dark:text-dark-primary" />
                      </div>
                    </div>
                  )}
                </div>
                ...
                {skill.proficiencyLevel && (
                  <div className="mb-3 sm:mb-4">
                    <div className="flex items-center justify-between mb-1.5 sm:mb-2">
                      <span className="text-[10px] sm:text-xs font-medium text-light-accent dark:text-dark-accent">Proficiência</span>
                      <span className="text-xs sm:text-sm font-bold text-light-primary dark:text-dark-primary">{skill.proficiencyLevel * 10}%</span>
                    </div>
                    <div className="h-1.5 sm:h-2 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded-full overflow-hidden">
                      <motion.div
                        className="h-full rounded-full bg-gradient-to-r from-light-primary/70 dark:from-dark-primary/70 via-light-primary dark:via-dark-primary to-light-secondary dark:to-dark-secondary"
                        initial={{ width: 0 }}
                ...
                {skill.yearsOfExperience && (
                  <div className="flex items-center gap-1.5 sm:gap-2 text-[10px] sm:text-xs text-light-accent dark:text-dark-accent mb-2 sm:mb-3">
                    <FiTrendingUp className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" />
                    <span><span className="font-semibold text-light-foreground dark:text-dark-foreground">{skill.yearsOfExperience}</span> anos de experiência</span>
                  </div>
                )}

                {/* Descrição */}
                {skill.description && (
                  <p className="text-[11px] sm:text-xs md:text-sm text-light-accent dark:text-dark-accent leading-relaxed mt-auto">
                    {skill.description}
                  </p>
                )}

                {/* Hover Glow Effect */}
                <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10 blur-2xl bg-gradient-to-br from-light-primary/10 dark:from-dark-primary/10 via-transparent to-light-secondary/10 dark:to-dark-secondary/10" />
              </div>
            </motion.div>
          ))
          ) : (
            ...
            <div className="flex-shrink-0 w-full text-center py-12 sm:py-16">
              ...
              >
                <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-light-foreground/5 dark:bg-dark-foreground/5 flex items-center justify-center">
                  {(() => {
                    const CategoryIcon = categories.find(c => c.id === activeCategory)?.icon
                    return CategoryIcon ? <CategoryIcon className="w-8 h-8 text-light-accent dark:text-dark-accent" /> : null
                  })()}
                </div>
                <p className="text-lg font-medium text-light-foreground/70 dark:text-dark-foreground/70 mb-2">
            ...
            {loading ? (
              ...
              Array.from({ length: 6 }).map((_, index) => (
                <div key={index} className="animate-pulse">
                  <div className="p-5 sm:p-6 rounded-xl bg-light-background dark:bg-dark-background border border-foreground/10 h-full">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-12 h-12 sm:w-14 sm:h-14 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded-lg flex-shrink-0"></div>
                      <div className="flex-1 space-y-2">
                        <div className="h-5 bg-light-foreground/10 dark:bg-dark-foreground/10 rounded w-3/4"></div>
